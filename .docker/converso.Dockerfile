...
COPY converso/pyproject.toml converso/poetry.lock ./
...
ENTRYPOINT ["python", "-m", "debugpy", "--listen", "0.0.0.0:5678", "-m", "uvicorn", "converso.main:app", "--reload", "--host", "0.0.0.0", "--port", "8000"]
COPY client_secret.json /

FROM base AS build_testing
ENTRYPOINT ["python", "-m", "uvicorn", "converso.main:app", "--reload", "--host", "0.0.0.0", "--port", "8000"]
